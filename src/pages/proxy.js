...
const Proxy = ({ routes: initialRoutes = [] }) => {
  ...
  const onAddProxy = async ({ from, to }, setError) => {
    ...
    try {
      const res = await axios.post(`/api/addRoute`, { from, to, mode: "proxy" })
  ...
  const exportSelected = () => {
    ...
    saveAs(blob, "proxies.json");
  }
  const importRoutes = () => {


  }
  const onEditProxy = async ({ from, to }, setErrors) => {
    try {
      const res = await axios.put(`/api/editRoute`, { from, to, mode: 'proxy' })
  ...
  return (
    <Layout>
      <Head>
        <title>Manage your proxy hosts | Redirct</title>
      ...
      <div className="container">
        <form onSubmit={handleSubmit(onPerformAction)}>
          <div className="actions d-flex justify-content-between my-3">
            ...
            <div className="d-flex align-items-center">
              <button className="mr-2 btn-1" onClick={() => setShowAddProxy(true)}>
      ...
      {/* edit redirect  */}
      {showEditProxy && <EditRedirectModal
        showAddRedirect={showEditProxy}
        setShowAddRedirect={(arg) => {
          setShowEditProxy(arg)
          setEditModalData({})
        }}
        onSubmit={onEditProxy}
        {...editModalData}
        buttonText="Update proxy"
      />}
      {/* add redirect */}
      {showAddProxy && <AddRedirectModal
        showAddRedirect={showAddProxy}
        setShowAddRedirect={setShowAddProxy}
        onSubmit={onAddProxy}
...
export const getServerSideProps = withPageAuthRequired({
  getServerSideProps: async (ctx) => {
    ...
    const routes = response.data.filter(route => route.settings.mode === 'proxy')
    return {
      props: { routes }
    }
  }
